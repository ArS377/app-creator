...
Runtime events separate BluePrinted from an animated architecture diagram. The events come from the running app. The build view uses high-level status and a post-build architecture manifest because Replit MCP does not expose Agent's private generation stream.
...
- Version 1 supports apps created through BluePrinted. It does not analyze arbitrary repositories or every framework.
...
6. The publish tool returns a status that BluePrinted can verify before presenting the runtime URL.
...
```mermaid
flowchart LR
    U["User prompt"] --> C["BluePrinted control plane"]
...
- React actions wrapped with the BluePrinted browser helper
...
The preview gate tests the complete message and token handshake in the current stable Chrome, Safari, and Firefox releases. If framing is blocked, BluePrinted opens the app in a top-level synchronized window and runs the same strict-origin handshake through `window.opener`. The fallback refuses to run when the opener or origin does not match.
...
BluePrinted publishes through `publish_app` and verifies the result through `get_publish_status`. Before creation, the interface warns the user not to put credentials or private data in the prompt. It does not treat an editor URL as a public runtime URL.
...
```json
{
  ...
  "$id": "https://blueprinted.dev/schemas/manifest-v1.json",
  "type": "object",
...
{
  "$schema": "https://json-schema.org/draft/2020-12/schema",
  "$id": "https://blueprinted.dev/schemas/event-v1.json",
  "$defs": {
...
{
  "$schema": "https://json-schema.org/draft/2020-12/schema",
  "$id": "https://blueprinted.dev/schemas/investigation-v1.json",
  "type": "object",
  "additionalProperties": false,
...
Approved wireframe: the tutorial workspace reference generated during design planning.
...
BluePrinted ships from `/Users/aryasomasundaram/Documents/Github/app-creator` as a published Replit App. The public URL opens the tutorial sample immediately. Users connect their Replit account only in Version 1, when they choose to generate an app.
...
- You kept returning to "professional not gamy." That constraint shaped BluePrinted into a debugging interface rather than a code city.
